...
- Triggered by `issue_comment` (created) and runs only if the comment body starts with `@pochi-agent`
...
Comment on a PR with `@pochi-agent` followed by your request, e.g. `@pochi-agent review this code`, `@pochi-agent explain the changes in this PR`, `@pochi-agent suggest improvements`.
